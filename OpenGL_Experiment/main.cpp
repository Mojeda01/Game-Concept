...
bool mouseCaptured = true; // Track mouse capture state
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
    if (!mouseCaptured) return; // Only process mouse input if the mouse is captured
...
void processInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
        mouseCaptured = !mouseCaptured; // Toggle mouse capture state
        if (mouseCaptured) {
            glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
            firstMouse = true; // Reset firstMouse to avoid a large jump
        } else {
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Apply the transformations and lighting
